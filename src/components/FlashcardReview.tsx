...
export const FlashcardReview = ({ card, onRate, mode }: FlashcardReviewProps) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto">
      ...
      <div
        className={`relative w-full h-80 ${mode === "show" ? "cursor-pointer" : ""}`}
        onClick={handleFlip}
      ...
      {showRatingButtons && (
        <div className="mt-8 animate-slide-up">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <Button variant="success" className="flex flex-col h-auto py-4 gap-1" onClick={() => onRate("easy")}>
              <CheckCircle2 className="w-5 h-5" />
              <span className="text-sm font-semibold">Easy</span>
              <span className="text-xs opacity-80">Remembered well</span>
            </Button>
            <Button variant="warning" className="flex flex-col h-auto py-4 gap-1" onClick={() => onRate("confusing")}>
              <AlertCircle className="w-5 h-5" />
              <span className="text-sm font-semibold">Confusing</span>
              <span className="text-xs opacity-80">Almost forgot</span>
            </Button>
            <Button variant="secondary" className="flex flex-col h-auto py-4 gap-1" onClick={() => onRate("almost")}>
              <HelpCircle className="w-5 h-5" />
              <span className="text-sm font-semibold">Struggled</span>
              <span className="text-xs opacity-80">Took effort</span>
            </Button>
            <Button variant="destructive" className="flex flex-col h-auto py-4 gap-1" onClick={() => onRate("forgot")}>
